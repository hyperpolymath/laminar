...
// Replace laminar, laminar, {{DEPS}} with actuals
= laminar — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining laminar for
...
| `$PREFIX/share/laminar/`
| Data files, assets

| `$PREFIX/share/doc/laminar/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/laminar/config.toml`

Fallback: `$HOME/.config/laminar/config.toml`
...
Or via OPSM: `opsm update laminar`
...
[source,bash]
----
just install --prefix=/opt/laminar-instance1 --config=/etc/laminar/instance1.toml
just install --prefix=/opt/laminar-instance2 --config=/etc/laminar/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/laminar/issues
